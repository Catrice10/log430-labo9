...
def get_stocks():
    ...
    rows = session.execute(
        text("SELECT product_id, quantity FROM stocks ORDER BY product_id")
    ).fetchall()
    session.close()
...
def reset_stocks():
    ...
    session = _session()
    session.execute(text("UPDATE stocks SET quantity = 1000, version = 0 WHERE product_id = 1"))
    session.execute(text("UPDATE stocks SET quantity = 500,  version = 0 WHERE product_id = 2"))
    session.execute(text("UPDATE stocks SET quantity = 2,    version = 0 WHERE product_id = 3"))
    session.execute(text("UPDATE stocks SET quantity = 90,   version = 0 WHERE product_id = 4"))
    session.commit()
    session.close()
...
def cluster_nodes():
    """Show CockroachDB node status (useful for resilience activity)."""
    session = _session()
    try:
        rows = session.execute(
            text("SELECT node_id, address, is_available, is_live FROM crdb_internal.gossip_nodes")
        ).fetchall()
        session.close()
        return jsonify([
            {
